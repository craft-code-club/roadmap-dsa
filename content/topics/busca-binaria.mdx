...
Repare no formato da segunda coluna: multiplicar o array por mil não multiplica o trabalho por mil, **acrescenta dez passos**. É a mesma jogada que a [árvore de busca binária](/topico/bst) faz em memória e que um índice de banco de dados faz em disco.
...
Essa ideia tem nome, *binary search on the answer*, e rende um tópico próprio: veja [Busca Binária no Espaço de Respostas](/topico/busca-binaria-avancada). A mesma lógica de dividir para conquistar também aparece na [árvore de busca binária](/topico/bst), onde o descarte de metade acontece escolhendo um ramo em vez de mexer em índices.
...
Para consolidar, [Big O](/topico/big-o) tem um contador que roda busca binária e busca linear sobre o mesmo array lado a lado. E [Two Pointers](/topico/two-pointers) é a técnica vizinha mais próxima: também explora um array ordenado, também é O(n) ou melhor, e também morre se a ordem não existir.
