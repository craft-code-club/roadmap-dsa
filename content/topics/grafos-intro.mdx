...
**Com peso**: cada aresta carrega um custo, distância ou tempo. É o que faz [Dijkstra](/topico/dijkstra) existir.
</Cartao>
</Colunas>
...
Depois troque para **completo** e compare os dois custos: com o grafo cheio, a matriz para de desperdiçar. E ligue o modo **dirigido**: repare que a simetria da matriz some, o que revela uma coisa boa de saber, que num grafo não dirigido metade da matriz é cópia da outra metade.
...
Aqui está a ideia que mais rende em problema de entrevista, e que apareceu no encontro: **você quase nunca recebe um grafo**. Você recebe outra coisa e precisa enxergar o grafo dentro dela.
...
- **Dependências**: cada tarefa é um vértice, cada "A precisa de B" é uma aresta dirigida. É o [ordenamento topológico](/topico/topological-sort).
...
Uma diferença prática entre grafo e [árvore](/topico/arvores-binarias) muda o código de todo percurso: no grafo existe ciclo, então você pode voltar a um vértice já visitado e girar para sempre.
...
Numa árvore, esse `set` não existe, porque a estrutura garante que você nunca volta. Num grafo, esquecer dele é recursão infinita. É a diferença mais importante entre os percursos que você já conhece e os de [DFS e BFS em grafos](/topico/dfs-bfs), que é exatamente o próximo tópico.
...
A partir daqui, o caminho é [DFS e BFS em Grafos](/topico/dfs-bfs), que traz os dois percursos com o detalhe dos visitados, e depois os algoritmos de caminho mínimo: [Dijkstra](/topico/dijkstra) para pesos não negativos e [Bellman-Ford](/topico/bellman-ford) para quando existe peso negativo.
