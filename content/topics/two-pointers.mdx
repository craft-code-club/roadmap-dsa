...
**Confundir `esquerda < direita` com `esquerda <= direita`.** Para formar um **par** de índices distintos, use `<`, senão o elemento do meio soma com ele mesmo e `[3, 8]` com alvo 6 devolve um par que não existe. Para palíndromo também é `<`, porque o caractere central não precisa de comparação. O `<=` aparece em outro contexto, o da [busca binária](/topico/busca-binaria), onde o intervalo pode ter um elemento só.
...
**Chamar de Two Pointers o que tem nome próprio.** Se a distância entre os ponteiros define uma janela cujo conteúdo interessa, isso é [Sliding Window](/topico/sliding-window). Se um anda o dobro do outro, é lento e rápido. Reconhecer o nome certo é parte da resposta.
...
| interessa o **conteúdo** do trecho entre os ponteiros | não é isto, é [Sliding Window](/topico/sliding-window) | expandir pela direita, encolher pela esquerda |
...
Daqui, os vizinhos naturais são a [Sliding Window](/topico/sliding-window), que é o mesmo par de ponteiros com uma regra de janela, e as [tabelas hash](/topico/hash-table), que resolvem em O(n) o que aqui custou uma ordenação. Se a conta de complexidade ainda escorrega, vale revisitar o [Big O](/topico/big-o).
