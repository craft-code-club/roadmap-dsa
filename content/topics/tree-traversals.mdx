...
Esta é a árvore que foi desenhada no quadro no encontro, e é a que o visualizador usa:
...
Tem um jeito de nunca mais confundir as três ordens, e ele apareceu no encontro pela boca do Mateus, que tinha lido num artigo. Contorne a árvore com um lápis, no sentido anti-horário, começando à esquerda da raiz e sem tirar a ponta do papel. Você vai passar **três vezes** por cada nó: uma pela esquerda dele, uma por baixo, e uma pela direita.
...
Coloque `processa(no)` em **(A)** e você tem pré-ordem. Em **(B)**, em ordem. Em **(C)**, pós-ordem. O resto do código é byte por byte idêntico. Foi assim que apareceu no encontro, e é assim que vale a pena memorizar: não são três algoritmos, é um algoritmo com três pontos de saída.
...
<Callout tipo="aviso">
Em ordem só produz saída ordenada se a árvore for uma BST. Numa árvore binária qualquer, como a do encontro, em ordem devolve 4, 2, 5, 1, 6, 3, que não tem ordem nenhuma. O percurso não ordena a árvore: ele revela uma ordem que a BST já tinha.
</Callout>
...
O exemplo que apareceu no encontro é **apagar uma árvore de diretórios**. Você não consegue remover uma pasta que ainda tem arquivo dentro: precisa esvaziar primeiro e só então apagar a pasta. Isso é pós-ordem, e o mesmo raciocínio vale para liberar memória de uma árvore em linguagem sem coletor de lixo.

O segundo exemplo é mais bonito: **avaliar uma árvore de expressão**. Nela, folha é número e nó interno é operador. Foi montada no quadro assim:
...
<Callout>
A generalização vale a pena guardar: sempre que um problema vira uma árvore de dependências, em que um nó só pode ser resolvido depois de todos os filhos, a resposta é pós-ordem. No encontro citaram o Jira: subtarefa fecha antes da tarefa, que fecha antes do épico, que fecha antes da sprint.
</Callout>
...
Espaço é onde mora a diferença, e foi onde a discussão do encontro rendeu mais.
...
<Callout tipo="aviso">
**h não é log n.** Isso só vale em árvore balanceada. A árvore degenerada do visualizador, em que todo nó tem um filho só, tem altura igual ao número de nós: o O(h) do DFS vira O(n) e a recursão pode estourar a pilha. Manter h perto de log n é exatamente o trabalho que a [árvore de busca binária](/topico/bst) balanceada faz, e o motivo de ela existir.
</Callout>
...
Depois daqui, o caminho natural é [Árvores Binárias](/topico/arvores-binarias), que aprofunda a estrutura em si, e a [Árvore de Busca Binária](/topico/bst), onde o percurso em ordem deixa de ser curiosidade e vira a razão de a estrutura existir. Do lado de grafos, [DFS e BFS em Grafos](/topico/dfs-bfs) reaproveita exatamente estes dois esqueletos, com uma diferença só: lá existe ciclo, então você precisa marcar quem já visitou.
