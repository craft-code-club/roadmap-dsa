...
Essa dupla vale mais que qualquer definição formal: na hora da compra você está, ao mesmo tempo, **dentro de uma fila e segurando uma pilha**. Quando o que importa é a ordem de chegada, é fila; quando o que importa é o mais recente, é [pilha](/topico/pilhas).
...
A outra base clássica é a [lista encadeada](/topico/listas-ligadas). Cada elemento vira um nó com o valor e o endereço do próximo; a fila guarda um ponteiro para o `inicio` e outro para o `fim`.
...
1. monte o [prefix sum](/topico/prefix-sum) `P`, com `P[0] = 0` e `P[i+1] = P[i] + nums[i]`. A soma de um trecho vira uma subtração: `P[j] - P[i]`.
...
O que torna o 862 difícil é ele aceitar **números negativos**, e é exatamente por isso que a [Sliding Window](/topico/sliding-window) clássica não dá conta: com negativos, aumentar a janela pode diminuir a soma, então a janela não sabe quando parar de encolher. O deque monotônico sobre o prefix sum sabe.

Isso fecha o ciclo com a [Sliding Window](/topico/sliding-window): lá a janela guardava uma soma ou uma contagem, coisas que dá para atualizar somando e subtraindo nas bordas. Máximo não é assim: quando o máximo sai pela esquerda, não existe conta que recupere o segundo maior. O deque monotônico é a estrutura que resolve exatamente esse buraco.
...
<Callout tipo="aviso">
**Fila de prioridade não é fila.** Apesar do nome, ela não entrega por ordem de chegada, e sim por prioridade, e a implementação eficiente é um heap, não um buffer circular. É outro tipo abstrato, com outra estrutura embaixo, e vive no tópico de [Binary Heap](/topico/binary-heap).
</Callout>
...
6. **Shortest Subarray with Sum at Least K (862)** junta deque monotônico com [Prefix Sum](/topico/prefix-sum). É difícil de verdade, e é o teste de que o padrão ficou: a receita em três passos está no fim da seção do deque monotônico, e o resto é você.

Daqui, dois caminhos naturais. Um é voltar para [Pilhas](/topico/pilhas) e comparar as duas estruturas irmãs lado a lado, já que uma constrói a outra. O outro é seguir para os [Percursos em Árvore](/topico/tree-traversals) e para o [BFS em grafos](/topico/dfs-bfs), onde a fila deixa de ser o assunto e passa a ser a ferramenta: é ela que garante que os nós saiam na ordem dos níveis.
