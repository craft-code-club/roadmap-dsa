...
Repare que as duas são [pós-ordem](/topico/tree-traversals): o resultado do pai depende dos dois filhos, então o pai só é resolvido depois deles. Sempre que a resposta de um nó é função das respostas dos filhos, o percurso é pós-ordem, e você já sabe escrever a função antes de pensar nela.

<Callout>
Uma armadilha comum: no encontro apareceu a observação de que uma árvore em que todo nó tem um filho só é, na prática, uma lista encadeada. Isso é literalmente verdade, e é uma boa lente. A árvore não é melhor que a lista por ser árvore; ela é melhor **quando ramifica**. Sem ramificação, você pagou o custo de dois ponteiros por nó para ter exatamente o desempenho de uma lista.
</Callout>
...
As vantagens são reais: zero memória com ponteiros, e todos os nós contíguos, o que agrada muito o cache do processador (é o mesmo argumento que faz array ganhar de lista na prática, discutido em [Arrays e Listas](/topico/arrays)).
...
O detalhe cruel é que a degeneração não vem de código errado. Ela vem de dado ordenado, que é o caso mais comum do mundo real: inserir 1, 2, 3, 4, 5 numa BST sem balanceamento produz uma linha reta. Foi para resolver isso que apareceram AVL e rubro-negra, que rebalanceiam a cada inserção, e a [Skip List](/topico/skip-list), que resolve o mesmo problema por sorteio em vez de rotação.
...
O próximo passo natural é a [Árvore de Busca Binária](/topico/bst), que acrescenta a invariante de ordem e transforma essa estrutura numa ferramenta de busca de verdade. Se você ainda não viu como andar pela árvore, [Percursos em Árvore](/topico/tree-traversals) vem antes, e as [Árvores N-árias](/topico/n-ary-trees) generalizam a mesma ideia para qualquer número de filhos.
