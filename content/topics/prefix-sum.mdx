...
O desperdício é exatamente o mesmo que motiva a [Sliding Window](/topico/sliding-window): recalcular do zero uma soma que já foi calculada. A diferença é que aqui as consultas chegam em qualquer ordem, então não existe uma janela andando para frente que você possa aproveitar.
...
`P` passa a ter `n + 1` posições, e o significado de cada célula muda de lugar: `P[k]` vira "a soma dos **primeiros k** elementos". `P[0] = 0` é a soma de nenhum elemento, que é o que a matemática já dizia. É a mesma ideia do nó sentinela das [listas encadeadas](/topico/listas-ligadas): um elemento falso na ponta para o caso de borda deixar de ser caso de borda.
...
A leitura da linha do meio é: se o prefixo `prefixo - k` já apareceu antes, então existe um subarray terminando **aqui** cuja soma é exatamente `k`, porque a diferença entre os dois prefixos é `k`. É o LeetCode 560, e é a ponte natural entre este tópico e [Tabelas Hash](/topico/hash-table).
