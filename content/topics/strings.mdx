...
Cada `append` é O(1) amortizado, porque a lista é um array dinâmico que dobra de tamanho de vez em quando (o mesmo mecanismo do tópico de [Arrays](/topico/arrays)). O `join` soma os tamanhos, aloca o total exato e copia. Total: **O(n)**.
...
**Otimizar onde não dói.** Se a string tem 20 caracteres e a função roda uma vez por requisição, escolha o código mais legível. O Big O só cobra a conta quando a escala aparece, e o gargalo real pode estar no banco ou na rede. Continue medindo antes de reescrever, como no tópico de [Big O](/topico/big-o).
...
Daqui, o caminho natural é [Two Pointers](/topico/two-pointers), que resolve palíndromo e inversão com dois índices convergindo, e [Sliding Window](/topico/sliding-window), que domina os problemas de substring com janela variável. Se a dúvida for de vocabulário, entre "substring" e "subsequence", passe antes pelos [4 "sub"](/topico/subarray-substring-subsequence-subset).
