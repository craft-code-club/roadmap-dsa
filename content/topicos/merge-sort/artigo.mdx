...
Isso deixa a estrutura clara. O merge sort é um percurso em que o trabalho acontece **depois** de resolver os dois filhos, o que é exatamente a forma de um percurso em pós-ordem (vale comparar com [Percursos em Árvore](/topicos/tree-traversals)).
...
O `mid = lo + (hi - lo) // 2` merece a mesma atenção que em [Busca Binária](/topicos/busca-binaria): escrever `(lo + hi) // 2` funciona em Python e estoura o inteiro em linguagens de tipo fixo. É o mesmo bug, no mesmo lugar, pelo mesmo motivo.
...
É a mesma mecânica de [Two Pointers](/topicos/two-pointers), com uma diferença: os dois ponteiros vivem em **arrays diferentes** e andam em ritmos independentes, em vez de convergirem dentro de um mesmo array.
...
Esse O(n) é o que separa o merge sort do [heap sort](/topicos/heap-sort) e do [quick sort](/topicos/quick-sort), que ordenam dentro do próprio array. Numa lista de 100 milhões de inteiros, o buffer é literalmente outra lista de 100 milhões de inteiros.
...
Daqui, [quick sort](/topicos/quick-sort) mostra a outra forma de dividir e conquistar, que faz o trabalho na descida em vez da volta, e [heap sort](/topicos/heap-sort) mostra como conseguir a mesma garantia de O(n log n) sem gastar memória. Se a recursão ainda incomodar, vale voltar em [Recursão](/topicos/recursao): o merge sort é o exemplo mais limpo de "resolva os filhos, depois combine" que existe.
