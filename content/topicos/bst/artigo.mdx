...
Uma consequência que vale destacar: percorrer a BST [em ordem](/topicos/tree-traversals) (esquerda, nó, direita) devolve os valores **ordenados**, sempre.
...
- **[Skip List](/topicos/skip-list)**: resolve o mesmo problema sem rotação nenhuma, sorteando a altura de cada nó. Consegue O(log n) **esperado** em vez de garantido, com código muito mais simples.
...
Se você só precisa de "existe?" e "pegue pela chave", a [tabela hash](/topicos/hash-table) ganha: O(1) médio contra O(log n). A BST não está tentando competir nisso.
...
Daqui, [Trie](/topicos/trie) mostra uma árvore de busca que não compara valores inteiros, e sim caminha por prefixo. E do lado dos grafos, a mesma ideia de "descartar o que não pode conter a resposta" volta em [Dijkstra](/topicos/dijkstra), com outra roupa.
