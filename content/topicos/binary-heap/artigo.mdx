<Callout>
Toda vez que um sistema precisa responder **"qual é o próximo?"** enquanto a lista de candidatos muda o tempo todo, existe um heap por perto. Ele é a estrutura por trás da fila de prioridade, do [Dijkstra](/topicos/dijkstra), do escalonador do seu sistema operacional e do [heap sort](/topicos/heap-sort). E o mais bonito é que ele não usa ponteiro nenhum: é uma árvore que mora dentro de um array comum.
</Callout>
...
<Callout tipo="aviso">
Não confunda com a [árvore de busca binária](/topicos/bst). Lá a regra é horizontal (tudo à esquerda é menor, tudo à direita é maior) e o percurso em ordem devolve os valores ordenados. Aqui a regra é vertical, e **o array de um heap quase nunca está ordenado**. Um array ordenado é sempre um min-heap válido; a recíproca é falsa.
</Callout>
...
Vale reparar numa coincidência que não é coincidência: essa numeração por nível é exatamente o resultado de um [percurso em largura (BFS)](/topicos/tree-traversals) na árvore. Ler o heap do índice 0 ao último é caminhar nível por nível.
...
Rode as duas no visualizador abaixo. No modo remover, deixe rodar até o fim e olhe a saída: ela sai ordenada, o que já entrega a ideia do [heap sort](/topicos/heap-sort).
...
[Dijkstra](/topicos/dijkstra) e Prim ([MST](/topicos/mst)) são gulosos, e o passo guloso deles é "pegue o mais barato ainda não fechado". Trocar a varredura linear por um min-heap é o que leva o Dijkstra de O(V²) para O((V + E) log V).
</Cartao>
</Colunas>
...
Daqui o caminho natural é ver o heap virar algoritmo de ordenação em [Heap Sort](/topicos/heap-sort), e depois virar peça de outro algoritmo no [Dijkstra](/topicos/dijkstra). Se a comparação com as árvores ficou em aberto, [Árvore de Busca Binária](/topicos/bst) fecha o outro lado, e [Filas e Deques](/topicos/filas) mostra a fila comum que a fila de prioridade generaliza.
