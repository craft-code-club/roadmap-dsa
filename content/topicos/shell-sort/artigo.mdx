<Callout>
O shell sort é o que acontece quando alguém olha para o [insertion sort](/topicos/ordenacao-basica), identifica exatamente qual detalhe o torna lento, e troca **uma constante por uma variável**. O resultado é um algoritmo que cabe em dez linhas, não aloca um byte, não usa recursão, e ainda assim escapa do quadrático. Ele também é o único algoritmo clássico cuja complexidade de caso médio continua sem fórmula fechada conhecida.
</Callout>
...
O padrão é o mesmo do selection sort e do [quick sort](/topicos/quick-sort): **movimento longo custa estabilidade**. Os algoritmos estáveis do grupo, insertion e [merge sort](/topicos/merge-sort), são exatamente os que nunca movem um elemento por cima de outro sem compará-los.
