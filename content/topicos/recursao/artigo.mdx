...
Recursão não é um algoritmo nem uma estrutura de dados. É uma **técnica**, da mesma família de [Two Pointers](/topicos/two-pointers), [Sliding Window](/topicos/sliding-window) e [Prefix Sum](/topicos/prefix-sum): um jeito de organizar a solução. A definição cabe numa linha, **uma função que chama a si mesma**, e é exatamente por ser tão curta que ela engana. O conceito entra em cinco segundos; a fluência em ler uma execução recursiva na cabeça leva bem mais.
...
O que vem depois no roadmap é praticamente uma lista de aplicações: [percursos em árvore](/topicos/tree-traversals), [DFS em grafos](/topicos/dfs-bfs), [merge sort](/topicos/merge-sort), [quick sort](/topicos/quick-sort), [busca binária](/topicos/busca-binaria), [backtracking](/topicos/backtracking) e [programação dinâmica](/topicos/programacao-dinamica). Nenhum deles fica confortável enquanto a pilha de chamadas for uma caixa preta.
...
E a pilha é literalmente a estrutura [Pilha (Stack)](/topicos/pilhas): último a entrar, primeiro a sair. É por isso que a recursão desce toda antes de começar a subir.
...
Isso tudo é a porta de entrada da [Programação Dinâmica](/topicos/programacao-dinamica), e o nome bonito esconde uma ideia simples: **DP é recursão que para de repetir trabalho**.
...
Esse é o assunto do tópico seguinte, e ele tem página própria: [Recursão em Programação Funcional](/topicos/recursao-funcional).
...
**Maximum Depth of Binary Tree (104).** A recursão estrutural na forma mais limpa que existe: três linhas, caso base no `None`, e o `max` dos dois filhos mais um. É o gabarito mental que você vai reusar em [percursos em árvore](/topicos/tree-traversals). Se sair sem pensar, o próximo tópico do roadmap fica fácil.
...
Quando estes estiverem confortáveis, os próximos passos naturais são [Recursão em Programação Funcional](/topicos/recursao-funcional), para entender a otimização de cauda de verdade, e [Percursos em Árvore](/topicos/tree-traversals), onde a recursão deixa de ser exercício e vira ferramenta de trabalho.
