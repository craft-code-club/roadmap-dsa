...
As duas estruturas guardam a mesma coisa e entregam em ordens opostas. Trocar uma pela outra troca a resposta do problema inteiro, e é por isso que a [fila](/topicos/filas) tem um tópico só dela.
...
<Callout>
Repare no que **não** acontece no `pop`: a capacidade não diminui. Um array que cresceu para 1024 posições continua ocupando 1024 posições mesmo com a pilha vazia. A memória só volta quando você joga a pilha inteira fora. É o preço da memória contígua, e é o mesmo assunto do tópico de [Arrays](/topicos/arrays).
</Callout>

## Pilha sobre lista ligada: sem resize, com um ponteiro a mais

A outra implementação troca o array por nós encadeados, o assunto do tópico de [Listas Encadeadas](/topicos/listas-ligadas). Cada nó guarda um valor e um ponteiro para o nó de baixo, e a pilha guarda só o `head`, que é o topo:
...
**Toda recursão pode virar um laço com pilha explícita.** Trocar a call stack por uma pilha sua move a memória da stack para a heap e some com o limite de profundidade. É exatamente essa troca que aparece nos [percursos em árvore](/topicos/tree-traversals) e no [DFS em grafos](/topicos/dfs-bfs): a versão iterativa do DFS é a versão recursiva com um `stack = []` no lugar da chamada. Aumente o expoente no visualizador até 8 e olhe a altura máxima subir junto: é essa altura que estoura.
...
**Usar pilha onde o problema é fila.** Se a ordem que importa é a de chegada, LIFO devolve tudo ao contrário. E, em Python, `lista.pop(0)` para simular uma fila custa O(n) por remoção, porque desloca todo o resto: o certo é `collections.deque`, que é o assunto de [Filas e Deques](/topicos/filas).
...
Daqui, dois caminhos se abrem. Um é a estrutura irmã, a [fila](/topicos/filas), que troca LIFO por FIFO e resolve a outra metade dos problemas de ordem. O outro é a [recursão](/topicos/recursao), que é a call stack desta página vista de dentro, e a porta de entrada para árvores e grafos.
