<Callout>
O heap sort é o que acontece quando você pega o [binary heap](/topicos/binary-heap) e resolve levar a ideia até o fim: se remover o maior custa O(log n), remover n vezes ordena o array inteiro em O(n log n). O detalhe que faz dele um algoritmo de verdade, e não só um heap com laço em volta, é que ele consegue fazer isso **dentro do próprio array**, sem alocar um único byte a mais.
</Callout>
...
A fase 1 custa O(n). A fase 2 faz n - 1 rodadas, e cada uma tem uma descida que percorre no máximo a altura da árvore, ⌊log₂ n⌋. O total é `O(n) + O(n log n)`, e como numa soma fica só o termo dominante (a regra vale a pena revisitar em [Big O](/topicos/big-o)), o algoritmo é **O(n log n)**.
...
Daqui, [Dijkstra](/topicos/dijkstra) mostra o heap trabalhando dentro de outro algoritmo em vez de ordenar, e [Big O](/topicos/big-o) fecha o raciocínio de por que uma soma de O(n) com O(n log n) vira O(n log n). Se a estrutura ainda estiver nebulosa, vale voltar em [Binary Heap](/topicos/binary-heap): o heap sort não tem uma linha que não venha de lá.
