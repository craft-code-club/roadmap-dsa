...
A mudança na estrutura é pequena. A [árvore binária](/topicos/arvores-binarias) tem dois campos fixos:
...
Aqui está a parte tranquilizadora. Todo o [percurso em profundidade](/topicos/tree-traversals) que você já escreveu continua valendo, com uma substituição mecânica: onde havia duas chamadas, agora há um `for`.
...
<Callout tipo="aviso">
Isso não é uma limitação chata, é uma pista. Em ordem existia na árvore binária porque a estrutura tinha uma **posição do meio**, e era exatamente essa posição que a [BST](/topicos/bst) usava para guardar ordem. Quando você abre o grau, perde a posição do meio e perde a ordem total de graça junto. A B-tree recupera isso guardando as chaves **dentro** do nó, ordenadas, e intercalando filhos entre elas. Ou seja: ela não desiste da ordem, ela paga por ela de outro jeito.
</Callout>
...
Depois daqui, [Trie](/topicos/trie) é a n-ária mais útil do roadmap, e a [Árvore de Busca Binária](/topicos/bst) fecha o outro lado da história: o que acontece quando você mantém o grau em 2 e usa a posição para guardar ordem.
