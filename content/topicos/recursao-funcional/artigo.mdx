...
Guarde o formato, porque ele é a solução do **Valid Parentheses** do LeetCode, e é a mesma ideia de [Pilhas](/topicos/pilhas) vista pelo outro lado: onde a linguagem imperativa usa um `stack.push()`, a funcional passa a pilha adiante no argumento.
...
Depois disso, o caminho natural passa por [Listas Ligadas](/topicos/listas-ligadas), que é a estrutura em que tudo isto foi construído, por [Pilhas](/topicos/pilhas), que é a estrutura que a recursão usa sem você ver, e por [Programação Dinâmica](/topicos/programacao-dinamica), onde o acumulador vira tabela.
