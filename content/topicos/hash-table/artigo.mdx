...
Isso derruba um O(n²) para O(n) numa passada só. Compare com o [Two Pointers](/topicos/two-pointers), que resolve o mesmo problema em O(n) **sem memória extra**, mas exigindo o array ordenado. Duas ferramentas, dois trade-offs.
...
| "existe subarray somando k?" | a [soma de prefixo](/topicos/prefix-sum) já vista | quantas vezes / o índice |
...
Daqui, dois caminhos naturais: revisitar [Arrays e Listas](/topicos/arrays) e [Listas Encadeadas](/topicos/listas-ligadas), que são as duas peças de que a tabela hash é feita, e revisar a [Notação Big O](/topicos/big-o) para entender por que a diferença entre O(1) e O(n) é a diferença entre um sistema que escala e um que não.
