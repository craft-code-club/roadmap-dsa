import { getTopico, TOPICOS } from "@content/roadmaps";
...
// que o `content/topicos/index.ts` já tem.
...
//
// `TOPICOS`, e não `TOPICOS`: com a lista do roadmap aqui, os tópicos dos
// trilhas e os tópicos avulsos continuariam apontando `og:image` para
// `/topicos/<slug>/opengraph-image` (o Next emite a meta a partir do ARQUIVO
...
// O símbolo foi trocado por palavras no `content/topicos/index.ts`, que é onde estava a
    ...
    if (semFonte.length > 0) {
      throw new Error(
        `opengraph-image (${slug}): ${JSON.stringify(semFonte.join(""))} em "${campo}" está fora ` +
          ...
          "símbolo por palavras no texto do tópico, em content/topicos/index.ts ou content/roadmaps.ts."
      );
    }
  }
}
...
export default async function TopicoOpengraphImage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // `generateStaticParams` acima enumera exatamente `TOPICOS`, então isto só
